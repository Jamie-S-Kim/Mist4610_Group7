...
## Problem Description:
The task at hand is to build a relational database model that organizes and supports the many operational aspects of a professional E-Sports tournament organization. The Tournaments entity serves as the central entity, since tournaments are the primary events around which the company bases its operations. Each tournament is connected to the Teams and Players entities that register to compete, and the competition is carried out through scheduled Matches that determine the final outcomes. This database also allows us to capture the sponsors of E-Sports tournaments, how those sponsors are connected to specific tournaments, and the full process of managing and distributing prize pools. In addition, the model supports the financial side of tournaments by tracking sponsorship contributions, prize allocations, and payout transactions. Our goal is to accurately model these relationships, generate realistic sample data, populate the tables with that data, and run managerially relevant SQL queries that provide useful business insights into tournament performance, match outcomes, sponsorship ROI, team results, and prize distributions.
